...
class Cube:
    ...
    def __init__(self, cube_str):
        ...
        # self.pieces is a list containing the elements below
        self.pieces = self.faces + self.edges + self.corners
    ...
    # _slice(), _rotate_face(), _rotate_slice(), _rotate pieces() are created to set up for the actual moves (ie. L R U D B F)
    ...
    # This is the sequence of moves generated by the solver
    # that will be applied to the cube via self.move() (see line 39 in cfop.py)
    def sequence(self, move_str):
        """
        :param moves: A string containing notated moves separated by spaces: "L Ri U M Ui B M"
        """
        # getattr(self, name) returns the attribute 'name' ('name' is a variable, not the actual name of the attribute) in object 'self'
        # In this case, each 'name' is a move in move_str
        # which is then split to be performed individually by move()
        moves = [getattr(self, name) for name in move_str.split()]
        for move in moves:
            move() # each 'move' will be a letter (L, Li, R, R2, etc.). Hence 'move()' means L(), Li(), etc.

    # Find the piece using its color combination
    def find_piece(self, *colors):
        if None in colors:
            return
        # Iterating over self.pieces
    ...
    def scramble(self, move_str):
        self.sequence(move_str)
