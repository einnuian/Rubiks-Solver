...
# Return a double move
def _double(move):
    return move + '2'

def apply_repeat_four_optimization(moves):
    """ R, R, R, R --> Nothing """
    changed = False
    i = 0
    while i < len(moves) - 2:
        if moves[i] == moves[i+1] == moves[i+2] == moves[i+3]:
            ...
            i += 1
    if changed:
        apply_repeat_four_optimization(moves)
...
def apply_no_full_cube_rotation_optimization(moves):
    ...
    changed = False
    i = 0
    a = moves.index("4") # Only apply optimization to moves before PLL
    while i < len(moves[:a]):
        if moves[i] not in rots:
            i += 1
            continue

        for j in reversed(range(i + 1, len(moves[:a]))):
...
def apply_no_double_optimization(moves):
    """ R R --> R2 """
    changed = False
    i = 0
    while i < len(moves) - 2:
        if moves[i] == moves[i+1]:
            moves[i:i+2] = [_double(moves[i])]
            changed = True
        else:
            i += 1
    if changed:
        apply_no_double_optimization(moves)
...
def optimize_moves(moves):
    ...
    apply_repeat_four_optimization(result)
    apply_no_double_optimization(result)
    return result
