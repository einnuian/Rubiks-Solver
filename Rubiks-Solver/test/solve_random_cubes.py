...
def run():
    ...
    while True:
        ...
        if C.is_solved():
            ...
            successes += 1
            # Substract solver.moves and opt_moves by 7 because I added 0 -> 6 to self.moves in cfop.py
            avg_moves = (avg_moves * (successes - 1) + len(solver.moves) - 7) / float(successes)
            avg_time = (avg_time * (successes - 1) + duration) / float(successes)
            avg_opt_moves = (avg_opt_moves * (successes - 1) + len(opt_moves) - 7) / float(successes)
        else:
            failures += 1
